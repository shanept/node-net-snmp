...
function readVarbinds(buffer, varbinds) {
  ...
  while (1) {
    ...
    if (type == Constants.ObjectType.Boolean) {
      ...
      throw new Exceptions.ResponseInvalidError("Unknown type '" + type + "' in response");
    }
  ...
  for (var i = 0; i < varbinds.length; i++) {
    ...
    if (varbinds[i].type && varbinds[i].hasOwnProperty('value')) {
      ...
      if (type == Constants.ObjectType.Boolean) {
        ...
        if (bytes.length != 4)
          throw new Exceptions.RequestInvalidError("Invalid IP address '" + value + "'");
        ...
        throw new Exceptions.RequestInvalidError("Unknown type '" + type + "' in request");
      }
    } else {
      buffer.writeNull();
    }
...
function isConfirmed(pdu) {
  switch (pdu.type) {
    case Constants.PduType.GetRequest:
    case Constants.PduType.GetNextRequest:
    case Constants.PduType.SetRequest:
    case Constants.PduType.GetBulkRequest:
    case Constants.PduType.InformRequest:
      return true;
  }
...
function isUnconfirmed(pdu) {
  switch (pdu.type) {
    case Constants.PduType.GetResponse:
    case Constants.PduType.TrapV2:
    case Constants.PduType.Report:
      return true;
  }
...
function inWriteClass(pdu) {
  return (pdu.type === Constants.PduType.SetRequest);
}

function inResponseClass(pdu) {
  switch (pdu.type) {
    case Constants.PduType.GetResponse:
    case Constants.PduType.Report:
      return true;
...
function inNotificationClass(pdu) {
  switch (pdu.type) {
    case Constants.PduType.InformRequest:
    case Constants.PduType.TrapV2:
      return true;
...
function inInternalClass(pdu) {
  return (pdu.type === Constants.PduType.Report);
}
...
module.exports = {
  ...
  inReadClass: inReadClass,
  inWriteClass: inWriteClass,
  inResponseClass: inResponseClass,
  inNotificationClass: inNotificationClass,
  inInternalClass: inInternalClass,
};
