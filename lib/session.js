...
var isVarbindError	= Functions.isVarbindError,
	...
	isConfirmed		= Functions.isConfirmed;
...
Session.prototype.send = function (req, noWait) {
	...
	req.message.toBuffer()
		.then(function(prepared) {
			var message = prepared.outgoingMessage;
			me.dgram.send (message, 0, message.length, req.port, me.target,
...
Session.prototype.simpleGet = function (pduClass, feedCb, varbinds,
	...
	try {
		...
		var processingModelPrimativeParameters = {
			...
			PDU: pdu,
			expectResponse: isConfirmed(pdu),
			sendPduHandle: id,
		...
		if (this.version >= Constants.Version3) {
			...
			var message = new RequestMessage (this, processingModelPrimativeParameters);
		}
