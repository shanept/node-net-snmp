...
var USM = require('./auth/user-based');
...
function SNMP_process_v3_options(options) {
	...
	processed.id = options.id;
	processed.boots = options.boots;
	processed.time = options.time;
	...
	switch (processed.securityModel) {
...
var Session = function (target, community, options) {
	...
	if (this.version < Constants.Version3) {
		...
		setInterval(function() {
			...
			// If the time gets above 2147483647, we increase the boot counter
			// and wrap the timer around to 0.
			if (me.time == 2147483647) {
				me.time = 0;
				me.boots += 1;
			}
		}, 1000);
...
Session.prototype.simpleGet = function (pduClass, feedCb, varbinds,
	...
	try {
		...
		if (this.version >= Constants.Version3) {
			...
			var securityModelClass,
				modelOpts = this.options.securityModelOptions;

			switch (this.secuirtyModel) {
				case Constants.SecurityModel.USM:
					securityModelClass = 'USM';

					// Add these SNMP options to the USM security model
					modelOpts.engineId = this.id;
					modelOpts.boots = this.boots;
					modelOpts.time = this.time;

					break;
				case 0:						// No authentication
					securityModelClass = 'NoAuth';
					break;
			}

			// Instantiate security model
			var security = new this[securityModel](modelOpts);
