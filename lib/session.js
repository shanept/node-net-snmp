...
Session.prototype.simpleGet = function (pduClass, feedCb, varbinds,
	...
	try {
		...
		var processingModelPrimativeParameters = {
			...
			PDU: pdu,
			expectResponse: false,			// Default to false, we will figure this out based on PDU type
			sendPduHandle: id,
		...
		switch (pdu.type) {
			...
			case Constants.PduType.InformRequest:
				processingModelPrimativeParameters.expectResponse = true;
				break;
			case Constants.PduType.GetResponse:
			case Constants.PduType.TrapV2:
			case Constants.PduType.Report:
				processingModelPrimativeParameters.expectResponse = false;
				break;
			default:
				throw new Error("Unexpected PDU type: " + pdu.type);
		}

		if (this.version >= Constants.Version3) {
			var message = new V3RequestMessage (this, processingModelPrimativeParameters);
		} else {
