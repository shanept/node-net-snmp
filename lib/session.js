 ...
 ** Options processors
 **/
function SnmpProcessDeprecatedOptions(options) {
  var processed = {};
...
function SnmpProcessV3Options(options) {
  ...
  processed = SnmpProcessDeprecatedOptions(options);
...
var Session = function (target, community, options) {
  ...
  if (this.version < Constants.Version3) {
    ...
    options = SnmpProcessDeprecatedOptions(options);
    ...
    if (pdu.varbinds.length % (repeaters)) {
      ...
      while (i < pdu.varbinds.length) {
        for (var j = 0; j < repeaters; j++, i++) {
          ...
          } else if (!oidFollowsOid(requestOID, respOID)) {
            req.responseCb(new Exceptions.ResponseInvalidError("OID '"
                + reqOID
