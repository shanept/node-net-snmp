...
TrapPdu.prototype.toBuffer = function (buffer) {
  var uptime = this.upTime || Math.floor(process.uptime() * 100);

  buffer.startSequence(this.type);

  buffer.writeOID(this.enterprise);
  buffer.writeBuffer(new Buffer(this.agentAddr.split('.')), Constants.ObjectType.IpAddress);
  buffer.writeInt(this.generic);
  buffer.writeInt(this.specific);
  writeUint(buffer, Constants.ObjectType.TimeTicks, uptime);
