...
RequestMessage.prototype.prepareOutgoingMessage = function(pdu) {
	...
	header.msgID = LOWORD(this.session.boots) << 16 | LOWORD(this.session.reqCount);
	header.msgMaxSize = this.session.maxSize;				// Is there a better way?
	header.msgFlags = this.options.securityLevel;
	 ...
	 */
	var SecurityModelClassName = Constants.SecurityModel[this.options.securityModel];
	eval('var security = new ' + SecurityModelClassName + '(this)');	// Eww...
	var securityParameters = {
		messageProcessingModel: this.options.messageProcessingModel,
		globalData: header,
		maxMessageSize: this.session.maxSize,				// Is there a better way?
		securityModel: this.options.securityModel,
		securityEngineId: this.session.engineID,			// Is there a better way?
	...
	messagePromise.then(function(message) {
		...
		return {
			...
			destTransportAddress: me.destTransportAddress,
